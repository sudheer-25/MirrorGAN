...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        # 1. if there is not a file called bird_captions.pickle, create this file (which contains the index version of captions).
        # 2. if there is such a file, we simply read the info.
        self.filenames, self.captions, self.ixtoword, \
        self.wordtoix, self.n_words = self.load_text_data(data_dir, split)

        self.class_id = self.load_class_id(split_dir, len(self.filenames))  # load class id
